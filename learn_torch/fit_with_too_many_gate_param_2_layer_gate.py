...
class Net(nn.Module):
    def __init__(self):
        ...
        self.gate0 = nn.Linear(2, N_HID)
        self.gate1 = nn.Linear(N_HID, 2)
        self.dense0 = nn.Linear(2, N_HID)
        self.dense1 = nn.Linear(N_HID, 1)

    def forward(self, x):
        mask = torch.sigmoid(self.gate1(torch.relu(self.gate0(x))))
        x = mask * x
...
mask = torch.sigmoid(net.gate1(torch.relu(net.gate0(t_input))))
